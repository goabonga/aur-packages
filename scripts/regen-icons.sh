...
# `assets/aur-packages.svg`. Re-run after editing the master SVG and commit
# the outputs - the docs job builds the site from the checked-in files
# and never runs this script.
#
# Outputs:
#   docs/aur-packages.svg   site logo   (zensical `[project.theme].logo`)
...
SRC="$ROOT/assets/aur-packages.svg"
...
cp "$SRC" "$ROOT/docs/aur-packages.svg"
...
uv run --only-group favicon python "$ROOT/scripts/generate_favicon.py" \
  -i "$ROOT/docs/aur-packages.svg" \
  -o "$ROOT/docs/favicon.ico"

echo "regen-icons: docs/aur-packages.svg + docs/favicon.ico from $SRC"
